 ...
 * DetachEvent.java
 * 
 * $Id: DetachEvent.java,v 1.1 2007/06/25 07:24:33 srothkugel Exp $
 *  
...
/**
 * A detach event occurs when a device loses the connection to another device. 
 * A detach event does not necessarily mean that both directions of a link fail since
 * the devices may have different sending radii.
 */
public class DetachEvent extends DynamicEvent {

	private final static String VERSION = "$Id: DetachEvent.java,v 1.1 2007/06/25 07:24:33 srothkugel Exp $";
	...
	/**
	 * Constructs a new DetachEvent for the specified devices.
	 ...
	 */
	public DetachEvent(double time, DynamicScheduler dynamicScheduler, DeviceID sender, DeviceID receiver, DynamicEventListener dynamicEventListener) {
	...
	protected void handleInternal() {
		...
		dynamicEventListener.handleDetach(sender, receiver);
	}
}
