 ...
 * LinkCalculator.java
 * 
 * $Id: LinkCalculator3D.java,v 1.1 2007/06/25 07:24:32 srothkugel Exp $
 *  
 ...
 */
public class LinkCalculator3D implements MobilityDynamicSource {

	private final static String VERSION = "$Id: LinkCalculator3D.java,v 1.1 2007/06/25 07:24:32 srothkugel Exp $";
	 ...
	 */
	public LinkCalculator3D(Output output) {
	...
	private void insertLinkCommands(DeviceID address) {
		...
		if (info.isSuspended()&&!info.suspendChanged()||info.sendingRadius<0) return;
		...
		while(it.hasNext()) {
			...
			if(!key.equals(address)) {
				...
				if (!keyInfo.isSuspended()&&!info.isSuspended()&&keyInfo.getSendingRadius()>=0){
...
//					}
	...
	private void insertEnterLinkCommands(DeviceID address, double time) {
		...
		while(it.hasNext()) {
			...
			if(!key.equals(address)) {
				DeviceInfo keyInfo=addressDeviceInfoMap.getDeviceInfo(key);
				if (!keyInfo.isSuspended()&&keyInfo.getSendingRadius()>=0&&sendingRadius>0){
					LinearMotion value =keyInfo.getLinearMotion();
					if(linearMotion.isInReach(time, value, sendingRadius)) { // TODO: isInReach wurde mit isInReach2 ausgetauscht -> ist das korrekt so???
						commandStack.push(new AttachCommand(time,Math.min(linearMotion.getEndTime(),value.getEndTime()), address, key));
					}
					if(value.isInReach(time, linearMotion, addressDeviceInfoMap.getDeviceInfo(key).getSendingRadius())) { // TODO: isInReach wurde mit isInReach2 ausgetauscht -> ist das korrekt so???
	...
	private static class AddressDeviceInfoMap {
		private Map addressDeviceInfoMap;
	...
	private static class LinearMotion {
		...
		public ClippedLinkInfo getClippedLinkInfo(LinearMotion linearMotion, double radius) {
            ...
            MutablePosition c=getPosition(0);
            MutablePosition d=getPosition(1);
            MutablePosition a=linearMotion.getPosition(0);
            MutablePosition b=linearMotion.getPosition(1);
            MutablePosition v=b;
            v.sub(a);
            v.sub(d);
            v.add(c);
            ...
            double underSqrt1=u.getX()*v.getX()+u.getY()*v.getY()+u.getZ()*v.getZ();
            double underSqrt2=(v.getX()*v.getX()+
                    v.getY()*v.getY()+
                    v.getZ()*v.getZ());
            double underSqrt=underSqrt1*underSqrt1-
                            (u.getX()*u.getX()+
                             u.getY()*u.getY()+
                             u.getZ()*u.getZ()-
                             radius*radius)*
			...
			double s1 = -(underSqrt1 + sqrt)/underSqrt2;
			double s2 = -(underSqrt1 - sqrt)/underSqrt2;
