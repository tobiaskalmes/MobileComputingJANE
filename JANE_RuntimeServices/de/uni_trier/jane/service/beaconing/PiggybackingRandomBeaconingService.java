 ...
 * 
 * $Id: PiggybackingRandomBeaconingService.java,v 1.1 2007/06/25 07:24:00 srothkugel Exp $
 * 
 ...
 */
public class PiggybackingRandomBeaconingService extends PiggybackingGenericBeaconingService {
	...
	public static final IdentifiedServiceElement INITIALIZER = new BeaconingInitializer(
		...
		public void createInstance(ServiceID ownServiceID, InitializationContext initializationContext, ServiceUnit serviceUnit) {
			...
			serviceUnit.addService(new PiggybackingRandomBeaconingService(ownServiceID, linkLayerID, bd, bj, bcd, bed));
		}
...
//		}
	...
	public static ServiceID createInstance(ServiceUnit serviceUnit, double beaconDelta, double beaconJitter, double cleanupDelta, double expireDelta,boolean visualize) {
        ServiceID linkLayerService = serviceUnit.getService(LinkLayer.class);
        ServiceID ownServiceID = new StackedClassID(GenericBeaconingService.class.getName(), linkLayerService);
        Service randomBeaconingService = new PiggybackingRandomBeaconingService(ownServiceID, linkLayerService, beaconDelta, beaconJitter, cleanupDelta, expireDelta);
        return serviceUnit.addService(randomBeaconingService,visualize);
    }
    ...
    public PiggybackingRandomBeaconingService(ServiceID linkLayerService, double beaconDelta, double beaconJitter, double cleanupDelta, double expireDelta) {
     ...
     * @return 
     */
    public static ServiceID createInstance(ServiceUnit serviceUnit, ServiceUnit netUnit,double beaconDelta, double beaconJitter, double cleanupDelta, double expireDelta) {
        ServiceID linkLayerService = netUnit.getService(LinkLayer.class);
        ServiceID ownServiceID = new StackedClassID(GenericBeaconingService.class.getName(), linkLayerService);
        Service randomBeaconingService = new PiggybackingRandomBeaconingService(ownServiceID, linkLayerService, beaconDelta, beaconJitter, cleanupDelta, expireDelta);
        return serviceUnit.addService(randomBeaconingService);
    }
     ...
     */
    public PiggybackingRandomBeaconingService(ServiceID ownServiceID, ServiceID linkLayerService, double beaconDelta, double beaconJitter, double cleanupDelta, double expireDelta) {
	...
	public void setPromiscuous(boolean promiscuous) {
	    //we must always stay in promiscuous mode
