...
public class Medium {
   ...
   */
	...
	public  final  class ReserverQueue{
		public TreeMap prioReserveList;
		 ...
		 *
		 */
		private  class ReserveQueueEntry implements Comparable{
			private long time;
		 ...
		 */
		public void add(DeviceID reserver,long stamp, Medium medium){

			if (reservesMap.isEmpty()){
				networkStatistic.networkInUse(owner);
			}
			ReserveQueueEntry entry=new ReserveQueueEntry(stamp,reserver);//timestamp.next(),reserver);
			prioReserveList.put(entry,medium);
			Object object=reservesMap.put(reserver,entry);
            if (object!=null){
...
//			LinkedHashSet senderSet;
//			if (receiverSendersMap.containsKey(receiver)){
//				senderSet=(LinkedHashSet)receiverSendersMap.get(receiver);
//			}else {
//				senderSet=new LinkedHashSet();
//			}
...
//			if (receiverSendersMap.containsKey(receiver)){
//				LinkedHashSet senderSet=(LinkedHashSet)receiverSendersMap.get(receiver);
...
//			LinkedHashSet senderSet;
//			if (receiverSendersMap.containsKey(receiver)){
//				senderSet=(LinkedHashSet)receiverSendersMap.remove(receiver);
//			}else {
//				senderSet=new LinkedHashSet();
//			}
//			return senderSet;
...
//			if (receiverSendersMap.containsKey(receiver)){
//				return ((LinkedHashSet)receiverSendersMap.get(receiver)).contains(sender);
//			}
//			return false;
//		}
//
//	}
	...
	public void  reserveUnicast(DeviceID receiver, SendHandler sendHandler){
		...
		long stamp=timestamp.next();
		reserverQueue.add(owner,stamp,this);

		
		HashMap unicastReceiverMap=new HashMap(neighbors);
		...
		while (iterator.hasNext()){
			...
			medium.reserverQueue.add(owner,stamp,this);
		}
	...
	public void reserveBroadcast(SendHandler sendHandler){
		...
		while(iterator.hasNext()){
			...
			((Medium)iterator.next()).reserverQueue.add(owner,stamp,this);
		}
		operatingSystem.setTimeout(new ServiceTimeout(0) {
			public void handle() {
				checkSendOK();
	...
	public void unicastFinished(){
		...
		HashMap unicastReceivingNeighbors=new HashMap(neighbors);
	...
	public void addNeighbor(DeviceID receiver, Medium medium) {
		...
		if (reserveInfo!=null){
			...
			if (reserveInfo.isUnicastInfo()&&receiver.equals(reserveInfo.getReceiver())){
				medium.unicastSenderList.add(owner);
				HashMap newUnicastReceivers=new HashMap(medium.neighbors);
	...
	public void removeNeighbor(DeviceID receiver) {
		...
		if (reserveInfo!=null){
			...
			if (reserveInfo.isUnicastInfo()){
                DeviceID unicastReceiver=reserveInfo.getReceiver();
				if (receiver.equals(unicastReceiver)){
					medium.freeMedium(owner);
					medium.unicastSenderList.remove(owner);
					HashMap unicastReceivingNeighbors=new HashMap(medium.neighbors);
				...
				}else if (neighbors.containsKey(unicastReceiver)&&
                        !((Medium)neighbors.get(unicastReceiver)).neighbors.containsKey(receiver)){
                        //!medium.neighbors.containsKey(reserveInfo.getReceiver())){
		...
		if (unicastSenderList.contains(receiver)){
            ...
            HashMap unicastReceivingNeighbors=new HashMap(neighbors);
            unicastReceivingNeighbors.keySet().removeAll(medium.neighbors.keySet());
            unicastReceivingNeighbors.remove(receiver);
            Iterator iterator=unicastReceivingNeighbors.values().iterator();
            while(iterator.hasNext()){
                Medium currentMedium=(Medium)iterator.next();
                currentMedium.freeMedium(receiver);
            }
		}
        ...
        while (iterator.hasNext()){
            ...
            if (!address.equals(receiver)&&
                    neighbors.containsKey(address)&&
                    !((Medium)neighbors.get(address)).neighbors.containsKey(receiver)){
                    //!medium.neighbors.containsKey(address)&&!address.equals(receiver)){
     ...
     * TODO Comment method
     */
    public ReserverQueue getReserveQueue() {
        return reserverQueue;
